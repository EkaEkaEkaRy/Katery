...
pygame.display.set_icon(pygame.image.load("PDL.png")) # иконка

gamedisplay.fill((0,0,0)) # заливка экрана черным
...
while game:
    ...
    for event in events: # берем события по одному
        ...
        if pygame.key.get_pressed()[pygame.K_SPACE]: # построение графика
            ...
            x = x_begin
            find_y()
            while x <= x_end: # построение графика по точкам
                ox2 = ox
                ox += 0.1
                x2 = x
                y2 = y
